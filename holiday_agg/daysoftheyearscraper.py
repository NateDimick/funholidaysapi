...
month_days = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]  # dict seems like too much here
...
for month_num in range(0, 12):
    ...
    if len(m) == 1:
        m = "0" + m
    for day_num in range(0, month_days[month_num]):
        day = str(day_num + 1)
        if len(day) == 1:
            day = "0" + day
        

        url = f"https://www.daysoftheyear.com/days/2021/{m}/{day}/"  # url to scrape from daysoftheyear.com
        print(month, url)
...
with open("fun_holidays_4_2021.txt", 'a', encoding="UTF-8") as file:
    for row in rows:
